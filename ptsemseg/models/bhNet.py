...
class BHNet(nn.Module):
    def __init__(
        ...
        # downsampling
        self.uencoder1 = Uencoder(self.feature_scale, self.is_deconv, 2, self.is_batchnorm, filters)
        self.uencoder2 = Uencoder(self.feature_scale, self.is_deconv, 14, self.is_batchnorm, filters)
        self.uencoder3 = Uencoder(self.feature_scale, self.is_deconv, 4, self.is_batchnorm, filters)
        ...
        # final layer
        self.final = nn.Conv2d(3, n_classes, 1)  # height_S1, height_S2, height_POI

    def forward(self, inputs):
        # encoder 1 & 2 & 3
        conv10, conv11, conv12, conv13, center1 = self.uencoder1(inputs[:, :2, :, :])  # S1
        conv20, conv21, conv22, conv23, center2 = self.uencoder2(inputs[:, 2:16, :, :])  # S2
        conv30, conv31, conv32, conv33, center3 = self.uencoder3(inputs[:, 16:20, :, :])  # POI

        # decoder 1 & 2 & 3
        com_center = torch.cat([center2, center1], 1)
        final1 = self.udecoder1(conv10, conv11, conv12, conv13, center1)  # S1 height
        final2 = self.udecoder2(conv20, conv21, conv22, conv23, com_center)  # S2 height
        final3 = self.udecoder3(conv30, conv31, conv32, conv33, center3)  # POI height
        final4 = self.final(torch.cat([final1, final2, final3], 1))  # S1+S2+POI height
